package com.example.rubikcubedelt
...
class CameraPreview(
    ...
    SurfaceView(context), SurfaceHolder.Callback, PreviewCallback {
    ...
    fun saveCurrentBitmap(side: Int) {
        ...
        val bitmap =
            Bitmap.createBitmap(camImageWidth, camImageHeight, Bitmap.Config.ARGB_8888)
        val bitmapData = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            renderScriptNV21ToRGBA888(
                context,
                camImageWidth,
                camImageHeight,
                data
    ...
    fun resolveColors(): Array<Array<CharArray>> {
        ...
        for (i in centerColors.indices) {
            ...
            Log.d(
                "Center Saturation " + indexColors[i],
                "" + centerColors[i]!![1]
            )
            Log.d(
                "Center Value " + indexColors[i],
                "" + centerColors[i]!![2]
            )
        }
        val colors = Array(
            ...
            for (j in 0..2) {
                for (k in 0..2) {
                    ...
                    if (j == 1 && k == 1) {
                        ...
                        var minDiff =
                            Math.abs(hue - centerColors[i]!![0]).toInt()
                        for (l in centerColors.indices) {
                            val diff =
                                Math.abs(hue - centerColors[l]!![0]).toInt()
                            if (diff < minDiff) {
                                minDiff = diff
                                color = indexColors[l]
                            }
